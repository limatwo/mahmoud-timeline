...
(function () {
  ...
  function groupDuplicateLabels(timeline) {
    ...
    while (i < events.length) {
      ...
      if (j - i > 1 && label) {
        // Skip grouping for fragment events — the wrapper div has no fragment
        // class, so getLastVisibleEvent() always treats it as visible and pans
        // to it immediately, breaking the fragment-slide pan animation.
        const anyFragments = events.slice(i, j).some(function (e) {
          return e.hasAttribute('data-fragment-index');
        });
        if (!anyFragments) {
